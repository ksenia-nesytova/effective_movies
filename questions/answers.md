...
**ООП** - объектно-ориентированное программирование. Основной идеей является разделение программы на более мелкие части, объекты, каждая из которых будет иметь свою зону ответственности и сама будет регулировать свое состояние. Взаимодействие объектов происходит через их методы.

JS это язык прототипного наследования, т.е. каждый объект имеет объект-прототип и может наследовать его методы и свойства. 

**Отличия от классического ООП в JS:**
* Нет (настоящих) классов, есть только прототипы. Классы
* Можно наследовать только от одного объекта(нет множественного наследования)

**Ключевые принципы ООП:**
* **Инкапсуляция** - данные должны быть защищены от воздействий извне. Т.е. внутреннее состояние объекта должно быть скрыто, доступ к нему может быть обеспечен только через публичный интерфейс. Достигается с помомщью использования #приватных полей и мтеодов и замыканий.
```javascript
class Otter {
    // приватные переменная и метод не будут доступны вне класса, т.е. они инкапсулированы
    #genus = 'Lutra';
...
* **Наследование** - объект или класс могут получать(наследовать) свойства или методы другого объекта или класса. Это позволяет переиспользовать код и выстраивать иерархии классов.
```javascript
class Animal {
    greet() {
        console.log(`The animal says: 'Hi!'`);
    }
}

class Otter extends Animal {
    greet() {
        // переопределяем метод в наследуемом классе
        console.log(`The otter says: 'Hi!'`);
    }
}
```

* **Полиморфизм** - поведение объектов, обладающих одинаковым интерфейсом, может быть реализовано по-разному
```javascript
class Animal {
    invite() { 
...
class Otter extends Animal {
    invite() {
        console.log('Introducing Mr Otter!');
    }
}

class Peacock extends Animal {
    invite() {
        console.log('Introducing Mr Peacock!');
    }
}
```

* **Абстракция** - выделяются и используются основные свойства и поведение объекта без необходимости знать детали реализации
```javascript
...
class CashPayment extends PaymentService {
        pay(amount) {
            console.log(`Will pay ${amount} in cash`);
    }
}
```
